...
This script contains the main routine of an SIA ice flow model that employs UPSURGING, described in Appendix C in:
...
#################################################################3
def custom_descrete_colorbar(minval,maxval,bins,colormap):
	...
	bounds = np.logspace(minval,maxval,bins+1)
	bounds = np.linspace(minval,maxval,bins+1)
...
def make_fig_endstate(pdf_name,bedrock_eff,ice,surf_elev):
	...
	# colorbar
	colormap, dummy = custom_descrete_colorbar(hmin,hmax,nn,plt.cm.jet)
...
def write_ts(cumul_time_series_file_name,time_now,ice,vol_error_cum):
	# append time stamp, ice volume and cumulative volume error to time series file
...
def initialize_model(input_bedrock_file,dx,iice):
	# initialize arrays for relaxed bedrock (bedrock0), current bedrock (bedrock_eff), domain size (nx,ny), resolution (dx), ice thickness (ice), surface elevation (surf_elev), mass balance (smb)
...
#------------------------------------------------------------------
def mass_balance(smbm, ice, surf_elev,sea_level,time_now,ela,mbal_grad,max_acc,nx,ny,dx):
...
def ice_flow_sia_muscl(H, S, B, Gamma, n, dx, dy, CFL, dt_uplim, dt_min):
	...
	# This subroutine stems from: https://github.com/alexjarosch/sia-fluxlim 
	# I adapted variable names such it can be used with the rest of my code (2021-02-15)
...
def ice_flow_sia_m2( ice,  surf_elev,  bedrock, CFL, dt_uplim, dt_min, use_upsurging, n, dx, nx, ny, omega):
	...
	if (use_upsurging):
		# cap surface differences with the upstream ice thickness
	...
	ice_new_raw[0:nx,0:ny] = ice[0:nx,0:ny] + dt/dx*( D_x[1:nx+1,0:ny] * h_x_c[1:nx+1,0:ny] - D_x[0:nx,0:ny] * h_x_c [0:nx,0:ny]    +    D_y[0:nx,1:ny+1] * h_y_c[0:nx,1:ny+1] - D_y[0:nx,0:ny] * h_y_c [0:nx,0:ny])
...
if __name__ == "__main__":
	...
	parser.add_argument('-ib', '--inputbedrock', default='None', type=str, help='input bedrock file')
	parser.add_argument('-iice', '--iice', default='False', type=str, help='initialize with ice from input bedrock file')
	parser.add_argument('-ibvn', '--inputbedrockvarname', default='topg', type=str, help='input bedrock variable name')
	parser.add_argument('-ys', '--ys', default=0.0, type=float, help='starting year')
	parser.add_argument('-ye', '--ye', default=20000.0, type=float, help='ending year')
	parser.add_argument('-of', '--output_freq', default=50.0, type=float, help='output rate [y]')
	parser.add_argument('-dx', '--resolution', default=5000.0, type=float, help='lateral resolution [m]')
	parser.add_argument('-smbm', '--smbm', default='eismint1fm', type=str, help='Surface mass balance model')
	...
	parser.add_argument('-exp_name', '--exp_name', default='spamspam', type=str, help='Experiment name shown on output directory')
	parser.add_argument('-model_choice', '--model_choice', default='m2', type=str, help='Chose SIA model version', choices=['m2', 'upsurging','muscl'])
...
#=============================================================
	...
	while(round(time_now,5) < ye):
		...
		smb_t0 = time.time() 
		smb = mass_balance(smbm,ice,surf_elev,sea_level,time_now,ela,mbal_grad,max_acc,nx,ny,dx)
		...
		if ( round(time_next_out,5) == round(time_now,5) ):
			...
			if run_time_figure:
				...
				data_plot = ax1[0].imshow(ice_masked, cmap='plasma_r', interpolation='nearest', origin='lower',vmin=0,vmax=1500)
		...
		print( '------------------------')
		print( 'Sea level: '+str(sea_level)+' m' )
		print( 'dt: '+str(dt) )
